...
int sameDim(Matrix& D,map<int,map<int,int>> Value_Functions,int& mod,vector<vector<int>>& Ef2){ //mod c'est la colonne à modifier, elle prend en sortie la valeur de la dimension+1
    int n,p;
    D.get_dim(n,p);
    while(mod != p){
//        cout<<"//==========="<<endl;
...
//        D.display();
        vector<int> labels=findLabel(D,Value_Functions,Ef2);
...
vector<int> EF2(Matrix& D,map<int,map<int,int>> Value_Functions,int nb_perl,int nb_player,vector<vector<int>>& Ef2){
    ...
    vector<int> labels=findLabel(D,Value_Functions,Ef2);  //Petite initialisation à la main pour éviter les bug.
    mod=findNext(D,labels);
    ...
    while(mod<=nb_player){
        test=sameDim(D,Value_Functions,mod,Ef2);
        if(test){
    ...
    labels=findLabel(D,Value_Functions,Ef2,true);
